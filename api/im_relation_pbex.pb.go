...
// return empty means pass
func (m *RelationReq) Validate() (errstr string) {
	if len(m.GetTarget()) != 24 {
		return "field: target in object: relation_req check value str len eq failed"
	}
	if m.GetTargetType() != "user" && m.GetTargetType() != "group" {
		return "field: target_type in object: relation_req check value str in failed"
	}
	return ""
}

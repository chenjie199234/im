...
const _WebPathRelationRelation: string ="/im.relation/relation";
...
export class RelationBrowserClientToC {
	...
	//timeout's unit is millisecond,it will be used when > 0
	relation(header: Object,req: RelationReq,timeout: number,error: (arg: LogicError)=>void,success: (arg: RelationResp)=>void){
		if(!header){
			header={}
		}
		header["Content-Type"] = "application/json"
		call(timeout,this.host+_WebPathRelationRelation,{method:"POST",headers:header,body:JSON.stringify(req)},error,function(arg: Object){
			let r=new RelationResp()
			r.fromOBJ(arg)
			success(r)
		})
	}

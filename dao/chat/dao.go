...
func (d *Dao) GetIndex(ctx context.Context, userid, chatkey string) (*model.IMIndex, error) {
	if index, e := d.RedisGetIndex(ctx, userid, chatkey); e == nil {
		...
		log.Error(ctx, "[dao.GetIndex] redis op failed", log.String("user_id", userid), log.String("chatkey", chatkey), log.CError(e))
	}
	...
	unsafeIndex, e := oneshot.Do("GetIndex_msg_recall_"+chatkey, func() (unsafe.Pointer, error) {
		...
		if e != nil {
			log.Error(ctx, "[dao.GetIndex] db op failed", log.String("chatkey", chatkey), log.CError(e))
			return nil, e
		}
		...
		go func() {
			if e := d.RedisSetIndex(ctx, userid, chatkey, msg, recall, 0); e != nil {
				log.Error(ctx, "[dao.GetIndex] update redis failed", log.String("user_id", userid), log.String("chatkey", chatkey), log.CError(e))
			}
		}()
	...
	unsafeAck, e := oneshot.Do("GetIndex_ack_"+chatkey+"_"+userid, func() (unsafe.Pointer, error) {
		...
		if e != nil {
			log.Error(ctx, "[dao.GetIndex] db op failed", log.String("user_id", userid), log.String("chatkey", chatkey), log.CError(e))
			return nil, e
		}
